...
def multiple_samples_collate_ego4d(batch, fold=False):
    """
...
def main(args, ds_init):
    ...
    if global_rank == 0 and not args.debug:
        wandb.init(project=args.project, config=vars(opts), id=args.wandb_id, resume="must" if args.wandb_id else None)
    ...
    dataset_train = StateChangeDetectionAndKeyframeLocalisation("train", args=args, pretrain=False)
    if args.disable_eval_during_finetuning:
        dataset_val = None
    else:
        dataset_val = StateChangeDetectionAndKeyframeLocalisation("val", args=args, pretrain=False)
